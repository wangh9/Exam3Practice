...
def integers(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        for b in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][b]) is int:
                list = list + [sequence_of_sequences[k][b]]
    return list
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k])is str:
            for b in range(len(sequence_of_sequences[k])):
                if sequence_of_sequences[k][b].isupper():
                    string1 += sequence_of_sequences[k][b]
    return string1
# ----------------------------------------------------------------------
