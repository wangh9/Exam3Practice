...
def run_test_zero_changer():
    ...
    test3 = ([0], [0,0,0,0,0,0,0], [4, 4, 4], [4, 0, 4])
    expected3 = ([1], [2,3,4,5,6,7,8], [4, 4, 4], [4, 9, 4])
    zero_changer(test3)
    print()
    print('Test 3:')
...
def zero_changer(tuple_of_lists):
    ...
    for k in range(len(tuple_of_lists)):
        for b in range(len(tuple_of_lists[k])):
            if tuple_of_lists[k][b] == 0:
                tuple_of_lists[k][b] = num
                num += 1
